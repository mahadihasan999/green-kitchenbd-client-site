...
const FoodDetailScreen = () => {
    ...
    return (
        <main className="max-w-screen-xl mx-auto px-6 my-16">
            <Back />
            {foods?.filter(item => item.name === name)?.map((food) => (
                <div className="flex flex-col justify-center items-center h-screen">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-10" key={food.id}>
                        ...
                        <div className="order-2 md:order-1 lg:order-1 flex flex-col justify-center">
                            ...
                            <p className="text-center md:text-left lg:text-left text-sm poppins text-gray-500 leading-relaxed select-none">{food.details}</p>
                        ...
                        <div className="order-1 md:order-2 lg:order-2">
                            <img  src={`https://res.awcbd.org/storage/menu/${food.image}`} className="w-3/4 md:w-3/4 lg:w-full mx-auto" alt="food" />
                        </div>
                    </div>
                </div>
            ))}
